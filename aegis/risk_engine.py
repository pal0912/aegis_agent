...
provenance taint status, capability severity, semantic alignment, and behavioral anomaly scores
to drive tri-state authorization.
"""

import logging
from typing import Any, Dict, Optional, Tuple

from aegis.types import BehavioralState, Capability, PolicyVerdict
...
class RiskEngine:
    ...
    # Baseline component weights (summing to 1.0)
    W_DETECTOR = 0.35
    W_TAINT = 0.20
    W_CAPABILITY = 0.25
    W_SEMANTIC = 0.20
    ...
    def calculate_risk(
        ...
        Formula:
            Base = W1 * Detector + W2 * Taint + W3 * Capability_Severity + W4 * (1.0 - Intent_Similarity)
            If Behavioral > 0: R = (1.0 - W_BEH) * Base + W_BEH * Behavioral
            Else: R = Base
        ...
        Args:
            ...
            behavioral_score: Anomaly score from the behavioral guard [0.0, 1.0].
            behavioral_state: State from the behavioral guard (NORMAL, SUSPICIOUS, ANOMALOUS, CRITICAL).
        ...
        is_critical_behavior = behavioral_state == BehavioralState.CRITICAL or behavioral_score >= 0.95
        if canary_tripped or not transition_valid or security_override or is_critical_behavior:
            breakdown = {
                ...
                "behavioral_component": self.W_BEHAVIORAL * max(0.0, min(1.0, behavioral_score)),
        ...
        norm_behavior = max(0.0, min(1.0, behavioral_score))
        c_behavioral = self.W_BEHAVIORAL * norm_behavior

        base_score = c_detector + c_taint + c_capability + c_semantic
        if norm_behavior > 0.0:
            raw_score = ((1.0 - self.W_BEHAVIORAL) * base_score) + c_behavioral
        else:
        ...
        breakdown = {
            ...
            "behavioral_component": round(c_behavioral, 4),
        }

        # 3. Tri-State Decision Logic
